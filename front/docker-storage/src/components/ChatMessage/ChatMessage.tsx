    ...
    return (
        <div style={{display:'flex', flexDirection: isOwnMessage ? 'row' : 'row-reverse'}}>
            ...
            <div style={{
                ...
                <Background bg_color={color.white} flex_direction={'row'} flex_alignItems={"stretch"} flex_justifyContent={isOwnMessage ? 'flex-start' : 'flex-end'}>
                    <p style={{margin:'10px', color:color.black, textShadow:'none'}}>{children}</p>
                </Background>
            </div>
        </div>
    );
