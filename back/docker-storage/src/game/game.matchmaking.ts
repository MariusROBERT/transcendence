...
export class GameMatchmaking {
  ...
  getGame(p1: number, p2: number | undefined = undefined): gameRoom {
    if (p2 == null)
      return this.controller.games.find(
        (g) => g.playerIds[0] == p1 || g.playerIds[1] == p1,
      );
    return this.controller.games.find(
      (g) =>
  ...
  async endGame(id: number) {
    ...
    // leave the room
    (await this.controller.gateway.getSocketFromUser(game.playerIds[0]))?.leave(
      game.room,
    );
    (await this.controller.gateway.getSocketFromUser(game.playerIds[1]))?.leave(
      game.room,
    );

    // remove the game from the controller's games
    this.controller.games = this.controller.games.filter(
      (g) => g.playerIds[0] != id || g.playerIds[1] != id,
    );
    // dev msg
    return 'game end';
  }
