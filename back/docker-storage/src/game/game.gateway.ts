...
export class GameGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleDisconnect(client: Socket) {
    const user = await this.UserService.getUserFromSocketId({
      socketId: client.id,
    });
  ...
  async joinQueueSpecial(@MessageBody() msg: { id: number }) {
    return this.controller.matchmaking.joinQueue(
      this.controller.queueSpecial,
      msg.id,
      true,
    );
  }
  ...
  async createRoom(p1: number, p2: number): Promise<string | undefined> {
    ...
    if (socketP1 === undefined || socketP2 === undefined) return undefined;
