...
export class GameService {
  ...
  bounce(state: State) {
    ...
    if (
      state.ball.x - size.ball / 2 < state.p1.x &&
      state.ball.y - size.halfBall < state.p1.y + size.halfBar &&
      state.ball.y + size.halfBall > state.p1.y - size.halfBar
    )
      state.dir = {
        x: state.ball.x - (state.p1.x - size.ball),
        y: state.ball.y - state.p1.y,
      };

    //collision with the right player
    if (
      state.ball.x + size.halfBall > state.p2.x &&
      state.ball.y - size.halfBall < state.p2.y + size.halfBar &&
      state.ball.y + size.halfBall > state.p2.y - size.halfBar
    )
      state.dir = {
        x: state.ball.x - (state.p2.x + size.ball),
        y: state.ball.y - state.p2.y,
      };
